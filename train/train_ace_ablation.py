...
def train(args):
    ...
    for step in range(start_step, args.max_steps):
        ...
        try:
            ...
            # Ablation 2026-04-30: --ablate_zsrc zeros z_src input to G (and to L_feat decoding below)
            # to test whether ACE's source-fidelity signal requires the source-clip input pathway.
            z_src_for_G = torch.zeros_like(z_src) if getattr(args, 'ablate_zsrc', False) else z_src
            ...
            # Ablation: when --ablate_zsrc, also zero z_src in L_feat decoding (consistent: no source signal anywhere)
            z_src_for_Lfeat = torch.zeros_like(z_src) if getattr(args, 'ablate_zsrc', False) else z_src
            x_src_norm = registry.decode_tokens(src_skel, z_src_for_Lfeat)
...
def main():
    ...
    parser.add_argument('--ablate_zsrc', action='store_true',
                        help='Ablation: zero out z_src input to G AND zero out z_src in L_feat decoding. '
                             'Tests whether ACE\'s source-fidelity signal requires the source-clip input pathway.')
