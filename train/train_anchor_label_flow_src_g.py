"""Train ANCHOR-Label-Flow-Src-Graph: graph-only variant (Codex R5 final blocker).

Same training protocol as train_anchor_label_flow_src.py but uses
AnchorLabelFlowSrcGraphGenerator (no skeleton ID embeddings, graph-only).

Forward signature change: forward(z_q, q, tgt_graph, src_z, src_graph, cluster_id, exact_id, ...)

Usage:
  python -m train.train_anchor_label_flow_src_g --run_name anchor_label_flow_src_g_v1 --max_steps 50000
"""
...
from model.actionbridge.generator_anchor_src_g import (
    AnchorLabelFlowSrcGraphGenerator, count_parameters,
)
...
def train(args):
    ...
    # Graph-only generator (no skel_id embeddings)
    ...
    for step in range(start_step, args.max_steps):
        ...
        for (sa, sb), items in groups_by_src.items():
            ...
            z_a = z_per_skel[sa][ra_t]
            z_b = z_per_skel[sb][rb_t]
...
def main():
    ...
    parser.add_argument('--run_name', type=str, default='anchor_label_flow_src_g_v1')
