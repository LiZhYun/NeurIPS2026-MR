"""DPG-SB-v3 training: completes tweak 3 of Codex's audit (cycle term).

Forked from train/train_dpg_sb_v2.py. Changes:
  1. CYCLE LOSS UN-ZEROED. v2 had `l_cycle = torch.zeros(...)` at line 268;
     v3 computes G(G(z_a, a→b), b→a) ≈ z_a in latent space, with
     w_cycle weighting (default 0.1 per Codex audit recommendation).
  2. Decoded-motion bone/contact/jerk penalty: NOT YET WIRED in this script.
     The Stage-A registry (model/moreflow/stage_a_registry.py) is available
     and could be integrated as v3.1 if the GPU-memory budget permits.
     Per the v2 status doc, completing only the cycle term (this script) is
     ~50% of tweak 3 and is expected to give a measurable AUC lift over v2.

Usage:
  python -m train.train_dpg_sb_v3 --run_name dpg_sb_v3 --max_steps 15000 --w_cycle 0.1

V2 historical context (forked from train_dpg_sb_v2.py):
  - SB in latent space (per-skel VQ z [8, 256])
  - Single SHARED conditioned bridge
  - Latent-space discriminator (loss_d ~line 272)
  - Retrieval-initialized noise
...
def train(args):
    ...
    for step in range(args.max_steps):
        ...
        # === Cycle loss (v3): G(G(z_a, a→b), b→a) ≈ z_a ===
        # Forward: estimate z_b_pred = z_start + v(z_start, t=0)
        if step >= args.adv_warmup:
            # z_b_pred already computed above for adversarial loss
            z_b_pred_for_cycle = z_b_pred
        else:
            t_zero = torch.zeros(z_a.shape[0], device=device)
            v_one = G(z_start, t_zero, src_tokens, aid, sa_idx, sb_idx)
            z_b_pred_for_cycle = z_start + v_one
        # Reverse: encode z_b_pred as new source, target is z_a, swap skel ids
        rev_src_tokens = G.encode_source(z_b_pred_for_cycle)
        # Use a fresh straight-line interpolant from a noise sample to z_a
...
def main():
    ...
    parser.add_argument('--w_cycle', type=float, default=0.1,
                        help='v3: 0.1 per Codex audit; v2 used 0.0 (cycle hard-zeroed)')
