...
def add_data_options(parser):
    ...
    group.add_argument("--objects_subset", default='all',
                       choices=['all', 'quadropeds' , 'flying', 'bipeds', 'millipeds',
                                'millipeds_snakes', 'quadropeds_clean', 'millipeds_clean',
                                'flying_clean', 'bipeds_clean', 'all_clean',
                                'train_v3', 'test_v3'],
                       type=str,
                       help="Object subset. train_v3/test_v3 = v3 benchmark split (10 held-out).")
...
def add_encoder_options(parser):
    ...
    group.add_argument("--enc_fsq_dims", default=4, type=int,
                       help="Number of FSQ dimensions K_fsq per spatial slot (FSQ ablation only).")
    group.add_argument("--enc_fsq_levels", default=5, type=int,
                       help="Number of quantization levels L per FSQ dim (FSQ ablation only).")
    group.add_argument("--enc_use_vae", default=True, type=lambda x: x.lower() != 'false',
                       help="Use variational (VAE) bottleneck. Pass 'false' for FSQ ablation.")
    group.add_argument("--enc_mode", default=None,
                       choices=['vae', 'fsq', 'direct', None],
                       help="Encoder mode: 'vae' (A1), 'fsq' (FSQ ablation), 'direct' (A2 baseline). "
                            "Overrides --enc_use_vae when set.")
    ...
    group.add_argument("--no_rest_pe", action='store_true',
                       help="Remove RestPE from encoder (no bone offset encoding). "
    ...
    group.add_argument("--lambda_inv", default=None, type=float,
                       help="View-consistency loss weight (default: 1.0). Set 0.0 for B1 ablation.")
    ...
    group.add_argument("--mask_ratio", default=None, type=float,
                       help="Max fraction of joints to mask in x_0 before noising. "
                            "Per-sample rate ~ U[0, mask_ratio]. Forces decoder to use tokens.")
    group.add_argument("--freeze_encoder", action='store_true',
                       help="[Stage 2] Freeze encoder weights. Use with --pretrained_encoder.")
    group.add_argument("--pretrained_encoder", default=None, type=str,
                       help="[Stage 2] Path to pretrained encoder state_dict from Stage 1.")
