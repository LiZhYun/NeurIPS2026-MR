...
class MotionDataset(data.Dataset):
    def __init__(self, opt, cond_dict, temporal_window, t5_name, balanced, split_files=None):
        ...
        self.split_files = set(split_files) if split_files is not None else None
        if self.split_files is not None:
            print(f"  Restricting to {len(self.split_files)} files from split")
        data_dict = {}
        ...
        for object_type in all_object_types:
            ...
            if self.split_files is not None:
                object_motions = [f for f in object_motions if f in self.split_files]
