"""ANCHOR-Label-Flow V5 inference: supervision-matched generative comparator.

Loads the trained ANCHOR-Label-Flow checkpoint (model/actionbridge/generator_anchor.py)
and produces per-query target-skeleton motion arrays in the same format the V5
evaluator (eval/benchmark_v3/eval_v5.py) expects.

Supervision-matched protocol (per Codex audit, the "single most damaging gap"):
  - cluster id  = I-5 RandomForest prediction on q_30d(source)  [same as ANCHOR]
  - exact id    = EXACT_ACTION_TO_IDX[q['src_action']]          [same as ANCHOR action label]
  - tgt_skel id, tgt_graph from cond_dict + skel_enc

Sampling: greedy single-step Euler  z_b = z_init + v(z_init, t=0). This matches
the v2/v3 LSB inference pattern; multi-step Euler is a simple drop-in upgrade
(see --n_euler_steps).

Decode: through frozen Stage-A target tokenizer (model/moreflow/stage_a_registry.py)
back to denormalized [T, J, 13] physical motion.

Output:
  eval/results/baselines/anchor_label_flow_v5/fold_<seed>/
    query_NNNN.npy   ([T, J, 13] denormalized)
    metrics.json     (per-query status + counts)

Usage:
  python -m eval.baselines.run_anchor_label_flow_v5 \
      --ckpt save/actionbridge/anchor_label_flow_v1/ckpt_final.pt \
      --fold 42 --max_queries 200
"""
...
from eval.baselines.run_i5_action_classifier_v3 import (
    train_classifier, CLUSTERS as I5_CLUSTERS, CLUSTER_TO_IDX as I5_CLUSTER_TO_IDX,
    featurize_q,
)
...
DEFAULT_CKPT = 'save/actionbridge/anchor_label_flow_v1/ckpt_final.pt'

# Cluster vocab MUST match training script
CLUSTERS_TR = sorted(ACTION_CLUSTERS.keys())
CLUSTER_TO_IDX = {c: i + 1 for i, c in enumerate(CLUSTERS_TR)}  # 0 = NULL
EXACT_ACTIONS = sorted({a for acts in ACTION_CLUSTERS.values() for a in acts})
EXACT_ACTION_TO_IDX = {a: i + 1 for i, a in enumerate(EXACT_ACTIONS)}  # 0 = NULL
...
def sample_z_b(G, z_init, q_t, tgt_id, tgt_graph, cluster_id, exact_id,
    ...
    """Greedy Euler sampling in latent flow.

    n_steps=1: z_b = z_init + v(z_init, t=0)  (single-step, fastest)
    n_steps=k: k-step Euler integration along the flow t in [0, 1]
    cfg_scale=w: classifier-free guidance with both labels (1.0 = no CFG).
    """
    z = z_init
    if n_steps == 1:
...
def main():
    ...
    parser.add_argument('--n_euler_steps', type=int, default=1,
                        help='Flow Euler sampling steps; 1 = greedy, 10-50 = better quality')
    parser.add_argument('--cfg_scale', type=float, default=1.0,
                        help='CFG scale; 1.0 = no CFG, >1 = stronger conditioning')
    ...
    # Load checkpoint
    print(f"Loading ANCHOR-Label-Flow ckpt: {args.ckpt}")
    G, skel_enc, skels, skel_to_id, ckpt_args = load_checkpoint(args.ckpt, device)
    sd_n_clusters = torch.load(args.ckpt, map_location='cpu', weights_only=False).get('n_clusters', None)
    sd_n_exact = torch.load(args.ckpt, map_location='cpu', weights_only=False).get('n_exact_actions', None)
    print(f"  ckpt skels: {len(skels)}, clusters: {sd_n_clusters}, exact_actions: {sd_n_exact}")
    ...
    if args.output_dir:
        ...
        out_dir = PROJECT_ROOT / f'eval/results/baselines/anchor_label_flow_v5/fold_{args.fold}'
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"Output dir: {out_dir}")

    # Load condition dict (contains mean/std/parents/offsets per skel)
    cond_dict = np.load(PROJECT_ROOT / 'dataset/truebones/zoo/truebones_processed/cond.npy',
                        allow_pickle=True).item()

    # Train I-5 classifier (same as ANCHOR uses)
    ...
    # Build a fname->qc-row index so we can look up the source's pre-computed features
    qc_meta = qc['meta']
    fname_to_qc_idx = {m['fname']: i for i, m in enumerate(qc_meta)}
    print(f"  classifier ready; {len(fname_to_qc_idx)} qc rows")
    # Inverse map cluster index -> cluster string (matches CLUSTERS_TR ordering used here)
    I5_IDX_TO_CLUSTER = {v: k for k, v in I5_CLUSTER_TO_IDX.items()}

    # Build skel-graph features for all skels we might need (target side)
    ...
    # Load Stage A registry for the unique target skeletons in this fold
    needed_targets = sorted({q['skel_b'] for q in queries})
    print(f"Loading Stage A tokenizers for {len(needed_targets)} target skels...")
    stage_a = StageARegistry(needed_targets, device=str(device))

    per_query = []
    n_token = 8  # MoReFlow Stage A token count (matches BridgeGenerator/AnchorLabelFlow)
    codebook_dim = 256
    t_total_0 = time.time()
    ...
    for i, q in enumerate(queries):
        ...
        rec = {'query_id': qid, 'cluster': cluster_gt, 'split': split,
               'skel_a': q.get('skel_a', ''), 'skel_b': skel_b,
               'src_action': src_action, 'status': 'pending'}

        try:
            # Sanity: target skel must be in our checkpoint and Stage A registry
            ...
            # Predict source cluster via I-5 RF (same as ANCHOR)
            ...
            if src_fname in fname_to_qc_idx:
                ...
                # Source clip is not in the precomputed Q cache; fall back to NULL cluster
                feat = None
            if feat is not None:
            ...
            # Exact-action: from V5 manifest's src_action (matches ANCHOR's filename label)
            exact_id_int = EXACT_ACTION_TO_IDX.get(src_action, 0)

            # Sample target z (ANCHOR-Label-Flow conditional inference)
            B = 1
            ...
            with torch.no_grad():
                z_b = sample_z_b(G, z_init, q_t=None, tgt_id=tgt_id,
                                 tgt_graph=tgt_graph, cluster_id=cid, exact_id=eid,
                                 n_steps=args.n_euler_steps, cfg_scale=args.cfg_scale)

            # Decode z_b through Stage A target tokenizer → normalized [B, T_dec, J, 13]
            with torch.no_grad():
                motion_norm = stage_a.decode_tokens(skel_b, z_b)
                motion_phys = stage_a.unnormalize(skel_b, motion_norm)  # [B, T, J, 13]

            sample = motion_phys[0].cpu().numpy().astype(np.float32)  # [T, J, 13]

            # Crop to target length matching V5 protocol (median of positives_cluster)
