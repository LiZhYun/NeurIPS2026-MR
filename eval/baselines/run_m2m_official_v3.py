...
Output: eval/results/baselines/m2m_official/v3_fold_{42,43}/query_NNNN.npy [T, J, 3]
"""
...
def main():
    ...
    if args.output_dir:
        ...
        out_dir = PROJECT_ROOT / f'eval/results/baselines/m2m_official/v3_fold_{args.fold}'
    ...
    for i, q in enumerate(queries):
        ...
        try:
            ...
            # Convert positive/adv .npy → .bvh names for exclusion
            forbidden = ({p['fname'].replace('.npy', '.bvh') for p in q['positives']}
                         | {a['fname'].replace('.npy', '.bvh') for a in q['adversarials']})
            ...
            # 6. Match output length to target's pos_median_T (per Codex)
