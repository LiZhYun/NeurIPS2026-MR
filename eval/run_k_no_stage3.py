"""Run Idea K pipeline WITHOUT Stage 3 (no frozen-AnyTop projection) on the
30 canonical eval pairs (idea-stage/eval_pairs.json).  This is an ablation of
run_k_pipeline_30pairs.py — only Stages 1 + 2 + bridge are applied; the
bridge's 13-dim output is saved directly.

Outputs:
  * per-pair motion -> eval/results/k_compare/K_no_stage3/pair_<id>_<src>_to_<tgt>.npy
  * metrics.json    -> eval/results/k_compare/K_no_stage3/metrics.json
  * classifier.json -> eval/results/k_compare/K_no_stage3/classifier_eval.json
"""
...
OUT_DIR = PROJECT_ROOT / 'eval/results/k_compare/K_no_stage3'
OUT_DIR.mkdir(parents=True, exist_ok=True)


# =================== helpers (copied from run_k_pipeline_30pairs.py) ===================
...
def run():
    ...
    for p in pairs:
        ...
        try:
            ...
            # --- NO Stage 3: save bridge output directly ---
            out_motion = motion_13
            rec['status'] = 'ok'
            rec['anytop_runtime'] = 0.0  # not run
    ...
    out = {
        'ablation': 'K_no_stage3',
        'note': 'Stage 3 (AnyTop DDIM projection) skipped; bridge output saved directly.',
    ...
    print(f'\n=== DONE (no-stage3): total {total_time:.1f}s, n_ok={out["n_ok"]}/{out["n_pairs"]}, failed={out["n_failed"]} ===')
...
def run_classifier_eval():
    """Run external classifier on K_no_stage3 outputs, comparing to source label
    and source classifier prediction. Saves classifier_eval.json.
    """
    import torch
    ...
    DATASET_DIR = PROJECT_ROOT / 'dataset/truebones/zoo/truebones_processed'
    ...
    # Source classifier predictions
    print('[classifier] computing source predictions')
    src_pred_by_pair = {}
    src_cache = {}
    for p in pairs:
        src_fname = p['source_fname']
    ...
    print(f'[classifier] K_no_stage3: valid {len(valid)}/{len(per_pair)}')

    # Stratification identical to k_action_accuracy.py:
    # absent > family_gap(near/moderate/extreme)
    ...
    out = {
        'method': 'K_no_stage3',
        'eval_set': str(PROJECT_ROOT / 'idea-stage/eval_pairs.json'),
        'n_pairs': len(pairs),
        'per_pair': per_pair,
        'summary': summary,
    }
    ...
    print('\n=== classifier summary (K_no_stage3) ===')
