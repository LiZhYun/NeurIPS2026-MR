...
def evaluate_character(args, character_name):
    ...
    # Threshold must match the distance metric: radians for rotation, L2 for position
    if use_pos:
        coverage_threshold = 0.5   # per-joint L2 distance (normalized position space)
    else:
        coverage_threshold = math.radians(20)  # 20 degrees (rotation space)
