...
from eval.metrics.ganimator_eval_kernel_py import prepare_group_cost
from eval.metrics.ganimator_eval_kernel_py import nn_dp as nn_dp_kernel
...
def group_cost_from_file(src_file, tgt_files, use_pos=False):
    ...
    # cost = torch.norm(src_pos - tgt_pos, dim=2)  # Ganimator calculation - irelevant for this project - deprecated!
    cost_fn = pos_avg_l2 if use_pos else pos_avg_cosine_distance
    cost = cost_fn(src_pos, tgt_pos)
