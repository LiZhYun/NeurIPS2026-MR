"""B1-no-gate baseline inference on 30 eval pairs.

Required because the earlier unified comparison lacks a pure B1 row (only
K/K_ikprior/etc. are present). This matches run_flexiact_gate_30pairs.py
exactly except it uses the unmodified AnyTopBehavior model (no gate MLP),
so the diff against flexiact_gate isolates the gate's effect.

Output:
    eval/results/k_compare/B1_no_gate/pair_<id>_<src>_to_<tgt>.npy
    eval/results/k_compare/B1_no_gate/metrics.json
"""
...
OUT_DIR = ROOT / 'eval/results/k_compare/B1_no_gate'
