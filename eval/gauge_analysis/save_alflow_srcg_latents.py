"""Extract z_b latents from AL-Flow-Src-G checkpoint (graph-only source variant, no skel IDs)."""
...
def main():
    ...
    print(f"[al-src-g] Loading ckpt: {args.ckpt}")
    ...
    print("[al-src-g] Loading registry...")
    ...
    print(f"[al-src-g] Processing {len(queries)} queries...")
    ...
    with torch.no_grad():
        for i, q in enumerate(queries):
            try:
                ...
                # Note Src-G signature: (z_q, q, tgt_graph, src_z, src_graph, cluster_id, exact_id) — no skel ids
                v_cond = G(z_init, t, tgt_graph, src_z, src_graph, cid, eid)
                z_b = z_init + v_cond
    ...
    print(f"[al-src-g] saved {len(triple_meta)} queries to {out_dir} in {time.time()-t0:.0f}s")
